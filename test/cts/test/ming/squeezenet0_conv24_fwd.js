describe('CTS', function() {
  ...
  it('squeezenet0_conv24_fwd', async function() {
    ...
    await fetch('./cts/test/ming/squeezenet0_relu22_fwd').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
    ...
    await fetch('./cts/test/ming/squeezenet0_relu24_fwd').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
    ...
    await fetch('./cts/test/ming/squeezenet0_conv24_weight.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
    ...
    await fetch('./cts/test/ming/squeezenet0_conv24_bias.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
