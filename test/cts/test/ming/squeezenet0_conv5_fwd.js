describe('CTS', function() {
  ...
  it('squeezenet0_conv5_fwd', async function() {
    ...
    await fetch('./cts/test/ming/squeezenet0_conv5_weight.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
    ...
    await fetch('./cts/test/ming/squeezenet0_conv5_bias.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
