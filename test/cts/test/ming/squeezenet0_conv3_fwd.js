describe('CTS', function() {
  ...
  it('squeezenet0_conv3_fwd', async function() {
    ...
    let op2value;
    let op3value;
    await fetch('./cts/test/ming/squeezenet0_conv3_weight.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
    ...
    await fetch('./cts/test/ming/squeezenet0_conv3_bias.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
