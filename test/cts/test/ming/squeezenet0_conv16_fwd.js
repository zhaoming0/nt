describe('CTS', function() {
  ...
  it('squeezenet0_conv16_fwd', async function() {
    ...
    await fetch('./cts/test/ming/squeezenet0_concat4').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
    ...
    await fetch('./cts/test/ming/squeezenet0_relu16_fwd').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
    ...
    let type4 = {type: nn.TENSOR_FLOAT32, dimensions: [384, 1, 1, 48]};
    ...
    await fetch('./cts/test/ming/squeezenet0_conv16_weight.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
    ...
    await fetch('./cts/test/ming/squeezenet0_conv16_bias.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
