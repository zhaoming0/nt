describe('CTS', function() {
  ...
  it('squeezenet0_conv15_fwd', async function() {
    ...
    await fetch('./cts/test/ming/squeezenet0_relu15_fwd').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
    ...
    await fetch('./cts/test/ming/squeezenet0_conv15_weight.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
    ...
    await fetch('./cts/test/ming/squeezenet0_conv15_bias.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
