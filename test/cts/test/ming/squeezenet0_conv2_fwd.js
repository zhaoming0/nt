describe('CTS', function() {
  ...
  it('squeezenet0_conv2_fwd', async function() {
    ...
    let op2value;
    let op3value;
    await fetch('./cts/test/ming/squeezenet0_conv2_weight.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
    ...
    await fetch('./cts/test/ming/squeezenet0_conv2_bias.txt').then((res) => {
      return res.text();
    }).then((text) => {
      let arr = text.split(',');
      let file_data = new Float32Array(arr.length);
